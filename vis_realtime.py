...
def interpolation(keypoints_1, keypoints_2):
    ...
    kpts_in1 = np.reshape(kpts_in1, (1, 1, 17, 2))
    kpts_in2 = np.reshape(kpts_in2, (1, 1, 17, 2))
    kpts_in3 = np.reshape(kpts_in3, (1, 1, 17, 2))
    kpts_in4 = np.reshape(kpts_in4, (1, 1, 17, 2))
    kpts_in5 = np.reshape(kpts_in5, (1, 1, 17, 2))
    kpts_in6 = np.reshape(kpts_in6, (1, 1, 17, 2))
    kpts_in7 = np.reshape(kpts_in7, (1, 1, 17, 2))
    kpts_in8 = np.reshape(kpts_in8, (1, 1, 17, 2))
    kpts_in9 = np.reshape(kpts_in9, (1, 1, 17, 2))
    kpts_in10 = np.reshape(kpts_in10, (1, 1, 17, 2))
    kpts_in11 = np.reshape(kpts_in11, (1, 1, 17, 2))
    kpts_in12 = np.reshape(kpts_in12, (1, 1, 17, 2))
...
def calculate_yaw_pitch_roll(vector1, vector2):
    vec1_normalized = normalize(vector1)
    vec2_normalized = normalize(vector2)

    yaw = np.arctan2(vec2_normalized[2], vec2_normalized[0]) - np.arctan2(vec1_normalized[2], vec1_normalized[0])
    pitch = np.arcsin(vec2_normalized[1]) - np.arcsin(vec1_normalized[1])
    roll = np.arctan2(vec2_normalized[1], vec2_normalized[0]) - np.arctan2(vec1_normalized[1], vec1_normalized[0])
...
def get_angles(x):
    ...
    raw = np.zeros((len(angles),))
    pitch = np.zeros((len(angles),))
    yaw = np.zeros((len(angles),))
    for i in range(len(angles)):
        yaw[i], pitch[i], raw[i] = calculate_yaw_pitch_roll(angles[i][0], angles[i][1])
        yaw[i] = yaw[i]*(180/math.pi)
        pitch[i] = pitch[i]*(180/math.pi)
        raw[i] = raw[i]*(180/math.pi)
                
    return yaw, pitch, raw
...
def parse_args():
    ...
    parser.add_argument('--cfg', type=str, default='/home/afm/StridedTransformer-Pose3D/demo/lib/hrnet/experiments/w48_384x288_adam_lr1e-3.yaml')
...
if __name__ == "__main__":
    ...
    pose_model.load_state_dict(torch.load('/home/afm/StridedTransformer-Pose3D/demo/lib/checkpoint/pose_hrnet_w48_384x288.pth'), strict=False)
    ...
    while(True):
        ...
        #     keypoints_aug = interpolation(keypoints_pre2, keypoints2)
        #     keypoints_aug.append(keypoints)
        # else:
        ...
        image_3d_original, kpts_3d = generate_3D_pose(keypoints_cache_r, image_2d, frame_count)
        image_3d = image_3d_original
        
        print('3d keypoints = ', kpts_3d)
        yaw, pitch, raw = get_angles(kpts_3d)
        ...
        if frame_count > 14:
            ...
            ax = plt.subplot(131)
            showimage(ax, frame)
            ax.set_title("Current Frame", fontsize = font_size)
            ...
            with open(os.path.join(output_dir, 'angles', f"angles_{frame_count:03}.csv"), "w") as f:
                f.write("yaw,pitch,raw\n")
                for i in range(18):
                    f.write(f"{yaw[i]},{pitch[i]},{raw[i]}")
                    f.write("\n")
                f.write("\n")
            print('Angles Recorded!!')
