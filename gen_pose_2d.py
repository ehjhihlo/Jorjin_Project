...
from lib.hrnet.lib.utils.transforms import get_affine_transform
...
def gen_vis_realtime(image_bgr, box_model, pose_model, cfg, device):
    last_time = time.time()
    image = image_bgr[:, :, [2, 1, 0]]
    ...
    if len(pred_boxes) >= 1:
        for box in pred_boxes:
            ...
            pose_preds, maxvals = get_pose_estimation_prediction(pose_model, image_pose, center, scale)
            # get_pose_estimation_prediction(pose_model, image_pose, center, scale)
            # keypoints, scores, _ = h36m_coco_format(pose_preds, maxvals)

        return pose_preds, maxvals
